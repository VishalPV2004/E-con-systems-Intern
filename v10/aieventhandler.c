...
int vehicle_AIStats(int count, char *vehicle_type) {
    ...
    logMessage("Vehicle AI JSON: %s", json_buffer);
...
int face_AIStats(int count) {
    ...
    strcpy(json_buffer, json_str);
    logMessage("Face AI JSON: %s", json_buffer);
...
int init_socket(char *IP) {
    ...
    logMessage("Connected to AI server at %s:8100", IP);
    return SUCCESS;
}
...
int postAIDataToCloud() {
    ...
    if (read(sockfd, &receive_packet, sizeof(PACKET)) <= 0) {
        logMessage("Socket read error");
        return FAILURE;
    }

    memcpy(&co_ord_data, receive_packet.message, sizeof(DETECTION));

    logMessage("Model file: %d", model_file);
    logMessage("Object Found");
    logMessage("co_ord_data.objects.cnt = %d", co_ord_data.objects.cnt);
